...
hamilton(N,X,Way):-
	search(N,X,[[n(X,0,0)]],[],Way).


% ?- search(6,a,[[n(a,0,0)]],[],Way).
search(N,_,_,[Way|_],Way):- 			% stop with path the way in front of Expanded
	Way=[n(_,N,_)|_],!. 				% if its length equals the number of nodes in graph
search(N,X,[C|Cands],Exp,Way):-
	expand(N,X,C,Cands,NewCands), 		% if not, expand best node so far = the element in front of current Cands
	search(N,X,NewCands,[C|Exp],Way). 	% and continue after expansion with the new Cands

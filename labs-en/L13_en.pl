...
% available and knows for each object its weight and the gain (value) obtained from transporting it to the destination. The task is 
% to determine which objects the person should carry so that the total gain is maximized, and the total weight is exactly equal to G.
